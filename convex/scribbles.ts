...
export const uploadScribble = mutation(
  async (
    { db, scheduler },
    { prompt, scribble }: { prompt: string; scribble: string }
  ) => {
...
export const getScribble = query(
  ({ db }, { scribbleId }: { scribbleId: Id<string> | undefined }) => {
    // if there is no id, stop the function
    if (scribbleId === undefined) {
      return null;
    }
    if (!scribbleId) {
      return null;
...
export const getScribbles = query(async ({ db }) => {
